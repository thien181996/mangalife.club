                ...
                <div class="m-portlet__body">
                    <div class="row">
                        <div class="col-lg-8">
                            <div class="row">
                                ...
                                <div class="col-lg-3">
                                    <select class="form-control m-input" name="select_manga_id" id="select_manga_id">
                                        <option value="0">-- Lựa chọn Manga --</option>
                                        @foreach($mangas as $manga)
                                            <option value="{{ $manga->id }}" {{ $manga_id == $manga->id ? "selected" : "" }}>{{ $manga->manga_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                    ...
                    <table class="table table-striped">
                        ...
                        @foreach($rates as $rate)
                            <tr>
                                <td>{{ $rate->getUserEmail() }}</td>
                                <td class="text-center">{{ $rate->rate_score }} / 10</td>
                                <td class="text-center">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton" x-placement="bottom-start" style="position: absolute; transform: translate3d(0px, 37px, 0px); top: 0px; left: 0px; will-change: transform;">
                                        <a class="dropdown-item rate_delete" href="#modal_rate_delete" value="{{ route('panel.deleteRate',['id'=>$rate->id]) }}"><i class="la la-trash"></i> Xóa</a>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
                    ...
                    {{ $rates->appends(['user_id'=>$user_id,'manga_id'=>$manga_id])->links() }}
                </div>
            </div>
            <!--end::Portlet-->
        </div>
...
@section('js')
    <script>
        ...
        $('#select_manga_id').on('select2:select', function (e) {
            let manga_id = e.params.data.id;
            if(manga_id != 0)
            {
                location.href = '?manga_id=' + manga_id;
            }
        });
    </script>
@endsection